...
if [[ "${ZOO_SERVER_ID}" == "0" ]]; then
  export ZOO_SERVERS=0.0.0.0:2888:3888,${SERVICE_NAME}-1:2888:3888,${SERVICE_NAME}-2:2888:3888
  echo "${ZOO_SERVERS}"
fi

if [[ "${ZOO_SERVER_ID}" == "1" ]]; then
  export ZOO_SERVERS=${SERVICE_NAME}-0:2888:3888,0.0.0.0:2888:3888,${SERVICE_NAME}-2:2888:3888
  echo "${ZOO_SERVERS}"
fi

if [[ "${ZOO_SERVER_ID}" == "2" ]]; then
  export ZOO_SERVERS=${SERVICE_NAME}-0:2888:3888,${SERVICE_NAME}-1:2888:3888,0.0.0.0:2888:3888
  echo "${ZOO_SERVERS}"
fi
